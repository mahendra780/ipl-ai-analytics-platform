...
def _render_batter_metrics(profile):
    ...
    col2.metric("Runs", profile["Runs"])
    col3.metric("Highest Score", profile["Highest Score"])

    col4, col5, col6 = st.columns(3)
    col4.metric("50s", profile["50s"])
    col5.metric("100s", profile["100s"])
    col6.metric("Strike Rate", profile["Strike Rate"])

    col7, col8 = st.columns(2)
    col7.metric("Fours", profile["Fours"])
    col8.metric("Sixes", profile["Sixes"])
...
def show_batter_profile(deliveries):
    ...
    batter_name = _select_batter(deliveries)
    profile = batter_profile(deliveries, batter_name)
...
def show_season_batter_analysis(deliveries):
    ...
    season = st.selectbox(
        "Select Season",
        sorted(deliveries["Season"].dropna().astype(str).unique()),
    )
...
def show_career_progression(deliveries):
    ...
    batter_name = _select_batter(deliveries)
    progression = batter_career_progression(deliveries, batter_name)
